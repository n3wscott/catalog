...
func init() {
	flag.IntVar(&options.Port, "port", 8080, "use '--port' option to specify the port for broker to listen on")
...
func runWithContext(ctx context.Context) error {
	...
	businessLogic, err := dummy.NewBusinessLogic(options.Options)
	if err != nil {
		return err
	}
